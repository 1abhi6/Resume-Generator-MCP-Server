"""
This module receives raw user text and produces an enhanced resume description using the project's LLM wrapper and common post-processing logic.
"""
...
def generate_resume_from_text(user_info: str, template_name: str):
    """
    Enhance a raw text resume fragment and apply common template logic.

    Parameters:
    - user_info: Raw text provided by the user (bio, experience, projects, etc.)
    - template_name: Identifier for the resume template to apply in post-processing

    Returns:
    - The processed response generated by get_common_logic (usually a dict/string
...
# response = generate_resume_from_text(text, template_name="default")
# print(response)
